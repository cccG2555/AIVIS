...
# 🚨 알람 및 실시간 프론트엔드 로그 연동용 상태 관리 객체 (broadcast_text 추가)
alarm_state = {
    "is_triggered": False,
    "msg": "",
    "broadcast_text": ""  # 🗣️ Ollama가 가공한 음성 안내 텍스트 저장용
}
...
# ======================================================================
# 🧠 [Ollama 음성 안내 멘트 생성 전용 함수] 💡 정석 위치에 배치
# ======================================================================
def get_ollama_voice_message(situation_text):
    prompt = f"""
    ...
    """
    try:
        # Jetson 환경에 맞는 경량화 모델 사용 (기본 llama3 기준)
        response = ollama.chat(model='gemma3:4b', messages=[{'role': 'user', 'content': prompt}])
        
        return response['message']['content'].strip()
    except Exception as e:
        print(f"[Ollama TTS Error]: {e}")
        return "비상 상황이 감지되었습니다. 즉시 확인 바랍니다."
...
def gen_frames():  
    ...
    while True:
        ...
        # ✅ 알람 최초 발생 시 Ollama 연동하여 음성 텍스트 문장 생성
        if (is_fall or is_sos) and not alarm_state["is_triggered"]:
            alarm_state["is_triggered"] = True
            active_errors = []
            situation_desc = ""
            
            if is_sos: 
                active_errors.append("[SOS DETECTED]")
                situation_desc = "작업자 SOS 구조 요청 수신호 감지"
            if is_fall: 
                active_errors.append("[FALL DETECTED]")
                situation_desc = "작업자 쓰러짐 및 무동작 사태 발생"
                
            alarm_state["msg"] = " ".join(active_errors)
            
            # 🗣️ Ollama를 호출하여 동적 경고 방송 멘트 생성 후 보관
            voice_msg = get_ollama_voice_message(situation_desc)
            alarm_state["broadcast_text"] = voice_msg
...
def index():
    ...
    <head>
        ...
        <script>
            ...
            // 브라우저 오디오 보안 제한 풀기용 터치 이벤트 추가
            document.addEventListener('click', function() {
                console.log("TTS 오디오 모듈 가동 성공");
            }, { once: true });

            // 🔄 1초마다 백엔드 상태를 통합 폴링하여 로그 업데이트 및 TTS 재생 처리
            setInterval(function() {
                fetch('/get_live_status')
                    ...
                    .then(data => {
                        ...
                        // 2. Ollama 가 작문한 경고 텍스트를 Web TTS 로 송출
                        if (data.is_triggered && data.broadcast_text) {
                            if (!isSpeaking && !window.speechSynthesis.speaking) {
                                isSpeaking = true;
                                
                                let utterance = new SpeechSynthesisUtterance(data.broadcast_text);
...
# ✅ 백엔드 상태(알람 여부, 로그, Ollama 문장)를 브라우저로 한 번에 넘겨주는 핵심 API 구역
@app.route('/get_live_status')
def get_live_status():
    global alarm_state, event_log_tracker
    logs = list(event_log_tracker["pending_logs"])
    event_log_tracker["pending_logs"].clear() 
    return jsonify(
        is_triggered=alarm_state["is_triggered"],
        logs=logs,
        broadcast_text=alarm_state["broadcast_text"]
    )
...
if __name__ == '__main__':
    # 외부 컴퓨터에서 접속할 수 있도록 host 설정을 0.0.0.0 으로 유지합니다.
